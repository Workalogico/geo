(function () {
  ...
  // Configuración de Mapbox — reemplazar con token público del proyecto
  const MAPBOX_TOKEN = window.WO_MAPBOX_TOKEN || '';
  ...
  const getTokens = () => {
    if (typeof document === 'undefined' || !document.documentElement) {
      return {
        ...
        danger: '#FF6B6B',
        success: '#10B981',
        accentPrimaryRgb: '255, 203, 0',
        accentSecondaryRgb: '89, 104, 234'
      };
    }
    ...
    return {
      ...
      accentPrimaryRgb: style.getPropertyValue('--wo-accent-primary-rgb').trim() || '255, 203, 0',
      accentSecondaryRgb: style.getPropertyValue('--wo-accent-secondary-rgb').trim() || '89, 104, 234'
    };
  };

  /**
   * Helper: Convierte hex a rgba para Mapbox que no soporta color-mix ni var()
   */
  ...
  const WO_MAP_THEMES = {
    ...
    yellow: {
      background: WO_COLORS.darkElevated,
      water: '#0A1628',
      building: WO_COLORS.darkSurface,
      roadPrimary: '#3A3A65',
      roadSecondary: '#2F2F50',
      highlight: '#FFCB00',
      accent: '#5968EA',
      poiPrimary: `rgba(${WO_COLORS.accentPrimaryRgb}, 0.8)`,
      poiSecondary: `rgba(${WO_COLORS.accentSecondaryRgb}, 0.7)`,
      textPrimary: WO_COLORS.text,
      textSecondary: '#CBD5E1',
      glow: `rgba(${WO_COLORS.accentPrimaryRgb}, 0.4)`
    },
    // Tema Tech (Azul dominante) - Más oscuro y técnico
    blue: {
      background: WO_COLORS.dark,
      water: '#050A14',
      building: WO_COLORS.darkElevated,
      roadPrimary: '#2A2A55',
      roadSecondary: '#1E1E40',
      highlight: WO_COLORS.blue,
      accent: WO_COLORS.yellow,
      poiPrimary: `rgba(${WO_COLORS.accentSecondaryRgb}, 0.8)`,
      poiSecondary: `rgba(${WO_COLORS.accentPrimaryRgb}, 0.7)`,
      textPrimary: '#E2E8F0',
      textSecondary: WO_COLORS.textSecondary,
      glow: `rgba(${WO_COLORS.accentSecondaryRgb}, 0.4)`
    }
  };
  ...
  const WO_MAPBOX_STYLE = {
    ...
    layers: [
      ...
      // Ciudad - Máximo contraste
      {
        ...
        paint: {
          "text-color": WO_COLORS.text,  // 15.3:1 ratio
          "text-halo-color": hexToRgba(WO_COLORS.dark, 0.9),
          "text-halo-width": 2,
          "text-halo-blur": 0.5
        }
      },
  ...
  class WoMapbox {
    ...
    createStructure() {
      ...
      <div class="wo-map-container animate-in">
        <div id="wo-mapbox-map-${this.containerId}" class="wo-mapbox-canvas-inner" style="width: 100%; height: 100%;"></div>
    ...
    async initMap() {
      try {
        ...
        this.map = new mapboxgl.Map({
          container: `wo-mapbox-map-${this.containerId}`,
          style: resolvedStyle,
    ...
    addLayers() {
      ...
      this.map.addLayer({
        ...
        paint: {
          ...
          'circle-stroke-color': WO_COLORS.text
        }
      });
    ...
    applyTheme(theme) {
      ...
      // Actualizar etiquetas (colores de texto y halos)
      const labelLayers = [
        'place-label-city', 'place-label-town', 'place-label-neighborhood',
        'place-label-suburb', 'road-label-primary', 'road-label-secondary', 'poi-label'
      ];

      const newTokens = getTokens();
      labelLayers.forEach(id => {
        if (this.map.getLayer(id)) {
          // Actualizar halo basado en el nuevo color de fondo
          this.map.setPaintProperty(id, 'text-halo-color', hexToRgba(newTokens.dark, 0.9));
          // La ciudad usa el color de texto brand, otras usan hardcoded/off-white que suele funcionar bien en dark
          if (id === 'place-label-city') {
            this.map.setPaintProperty(id, 'text-color', newTokens.text);
          }
        }
      });
    ...
    /**
     * Forzar re-cálculo de dimensiones (necesario en Reveal.js al cambiar slides)
     */
    resize() {
      if (this.map) {
        this.map.resize();
